...
export default function Home() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const keyframes: { center: number; target: number; scale: number }[] = [];
      ...
      for (let i = 0; i < keyframes.length - 1; i++) {
        if (
          ...
          bgScaleRef.current =
            keyframes[i].scale +
            (keyframes[i + 1].scale - keyframes[i].scale) * smooth;
          return;
        }
      }
    };
  ...
  return (
    <main className="relative min-h-screen text-white overflow-x-hidden">
      <AboutBackground offsetRef={bgOffsetRef} scaleRef={bgScaleRef} />
